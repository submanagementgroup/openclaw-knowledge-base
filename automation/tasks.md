---
domain: automation
topic: "Background Tasks: Activity Ledger for ACP Runs, Subagents, Cron, and CLI Operations"
type: concept
keywords:
  - background tasks
  - task tracking
...
related:
  ...
  - tools/subagents
source: automation/tasks.md
---

Background tasks are the activity ledger for work running outside your main session: ACP runs, subagent spawns, isolated cron executions, and CLI operations. Tasks do not schedule work — they track it.
...
- Subagent tasks: backing child session disappeared from the target agent store. Subagent tasks whose child session has a restart-recovery tombstone are marked `lost` instead of being treated as recoverable backing sessions.
...
## CLI reference
    ...
    Output columns: Task ID, Kind, Status, Delivery, Run ID, Child
