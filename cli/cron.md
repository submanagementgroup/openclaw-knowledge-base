---
domain: cli
topic: "CLI: openclaw cron Commands"
type: reference
keywords:
  - openclaw cron
  - cron CLI
...
> **Note:** Run `openclaw cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
...
> **Note:** Main-session jobs may only use `delivery.failureDestination` when primary delivery mode is `webhook`. Isolated jobs accept it in all modes.
...
> **Note:** One-shot jobs delete after success by default. Use `--keep-after-run` to preserve them.
...
> **Note:** `openclaw cron run <job-id>` force-runs by default. Use `--due` to keep the older "only run if due" behavior.
...
> **Note:** If the model is not allowed or cannot be resolved, cron fails the run with an explicit validation error instead of falling back to the job's agent or default model selection.
