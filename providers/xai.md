...
> **Note:** OpenClaw uses the xAI Responses API as the bundled xAI transport. The same
...
> **Note:** `grok-4.3`, `grok-4-fast`, `grok-4-1-fast`, and the `grok-4.20-beta-*`
...
> **Note:** OpenClaw uses xAI's REST image/video/TTS/STT APIs for media generation,
...
The bundled `xai` plugin registers video generation through the shared
    ...
    > **Note:** Local video buffers are not accepted. Use remote `http(s)` URLs for
    ...
    > **Note:** See [Video Generation](/tools/video-generation) for shared tool parameters,
...
The bundled `xai` plugin registers image generation through the shared
    ...
    > **Note:** xAI also documents `quality`, `mask`, `user`, and additional native ratios
...
The bundled `xai` plugin registers text-to-speech through the shared `tts`
    ...
    > **Note:** OpenClaw uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
...
The bundled `xai` plugin also registers a realtime transcription provider
    ...
    > **Note:** This streaming provider is for Voice Call's realtime transcription path.
...
The bundled xAI plugin exposes `code_execution` as an OpenClaw tool for
    ...
    > **Note:** This is remote xAI sandbox execution, not local [`exec`](/tools/exec).
