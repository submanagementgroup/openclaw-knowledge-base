...
> **Note:** OAuth setups use the `minimax-portal` provider id. Model refs follow the form `minimax-portal/MiniMax-M2.7`.
    

    > **Note:** Referral link for MiniMax Coding Plan (10% off): [MiniMax Coding Plan](https://platform.minimax.io/subscribe/coding-plan?code=DbXJTRClnb&source=link)
...
### Config example
    ...
    > **Note:** API-key setups use the `minimax` provider id. Model refs follow the form `minimax/MiniMax-M2.7`.
...
> **Note:** See [MiniMax Search](/tools/minimax-search) for full web search configuration and usage.
...
> **Note:** See [Model providers](/concepts/model-providers) for provider rules.
...
**MiniMax Search:** Web search configuration via MiniMax Token Plan.
