---
domain: providers
topic: "Anthropic / Claude Provider"
type: integration
keywords:
  - anthropic
  - claude
...
> **Note:** Anthropic staff told us OpenClaw-style Claude CLI usage is allowed again, so
...
> **Note:** Setup and runtime details for the Claude CLI backend are in [CLI Backends](/gateway/cli-backends).
    ...
    > **Note:** If you want the clearest billing path, use an Anthropic API key instead. OpenClaw also supports subscription-style options from [OpenAI Codex](/providers/openai), [Qwen Cloud](/providers/qwen), [MiniMax](/providers/minimax), and [Z.AI / GLM](/providers/glm).
...
OpenClaw's shared `/fast` toggle supports direct Anthropic traffic (API-key and OAuth to `api.anthropic.com`).
    ...
    > **Note:** - Only injected for direct `api.anthropic.com` requests. Proxy routes leave `service_tier` untouched.
...
Anthropic's 1M context window is beta-gated. Enable it per model:
    ...
    > **Note:** Requires long-context access on your Anthropic credential. Legacy token auth (`sk-ant-oat-*`) is rejected for 1M context requests — OpenClaw logs a warning and falls back to the standard context window.
...
**CLI backends:** Claude CLI backend setup and runtime details.
  

**Prompt caching:** How prompt caching works across providers.
