...
Set `mode: "cloud"` and point at your workflow file:
        ...
        > **Note:** Cloud mode defaults `baseUrl` to `https://cloud.comfy.org`. You only need to set `baseUrl` if you use a custom cloud endpoint.
...
Set the default video model to `comfy/workflow`:
    ...
    > **Note:** OpenClaw does not pass input videos into Comfy workflows. Only text prompts and single reference images are supported as inputs.
