---
domain: providers
topic: "InfeRRS Provider"
type: integration
keywords:
  - InfeRRS
  - inferrs provider
source: providers/inferrs.md
---
...
> **Note:** `inferrs` is currently best treated as a custom self-hosted OpenAI-compatible backend, not a dedicated OpenClaw provider plugin. You configure it through `models.providers.inferrs` rather than an onboarding choice flag. If you need a true bundled plugin with auto-discovery, see [SGLang](/providers/sglang) or [vLLM](/providers/vllm).
...
Some `inferrs` Chat Completions routes accept only string
    ...
    > **Note:** If OpenClaw runs fail with an error like:
...
If OpenClaw no longer gets schema errors but `inferrs` still crashes on larger
  ...
  > **Note:** For general help, see [Troubleshooting](/help/troubleshooting) and [FAQ](/help/faq).


## Related

**Local models:** Running OpenClaw against local model servers.
  

**Local model services:** Starting local model servers on demand for configured providers.
