---
domain: memory
topic: "LanceDB Memory Plugin: Vector Database Backend for High-Scale Memory Search"
type: procedure
keywords:
  - LanceDB
  - memory-lancedb
...
source: plugins/memory-lancedb.md
---

The memory-lancedb plugin provides a vector database backend for OpenClaw memory using LanceDB. It is an alternative to the builtin SQLite engine for high-scale or GPU-accelerated search.
...
`memory-lancedb` depends on the native `@lancedb/l
