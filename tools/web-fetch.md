...
> **Note:** If Firecrawl is enabled and its SecretRef is unresolved with no
  `FIRECRAWL_API_KEY` env fallback, gateway startup fails fast.


> **Note:** Firecrawl `baseUrl` overrides are locked down: hosted traffic uses
...
> **Note:** If no HTTP(S) proxy env var is configured, or the target host is excluded by
