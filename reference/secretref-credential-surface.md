---
domain: reference
topic: "SecretRef Credential Surface Reference"
type: reference
keywords:
  - SecretRef
  - credential surface
  - secrets reference
  - secret paths
source: reference/secretref-credential-surface.md
---
...
- In `openclaw.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:` marker strings are rejected on SecretRef credential paths; run `openclaw doctor --fix` to migrate valid markers.
