---
domain: gateway
topic: "Gateway Network Model and Bridge Protocol"
type: concept
keywords:
  - network model
  - bridge protocol
  ...
  - WebSocket bridge
  - connection model
source: ['gateway/bridge-protocol.md', 'gateway/network-model.md']
---
...
> **Note:** The TCP bridge has been **removed**. Current OpenClaw builds do not ship the bridge listener and `bridge.*` config keys are no longer in the schema. This page is kept for historical reference only. Use the [Gateway Protocol](/gateway/protocol) for all node/operator clients.
