---
domain: concepts
topic: "Multi-Agent Architecture: Subagents and Parallel Execution"
type: concept
keywords:
  - multi-agent
  - subagents
...
> **Note:** Never reuse `agentDir` across agents (it causes auth/session collisions). Agents
...
> **Note:** **Workspace note:** each agent's workspace is the **default cwd**, not a hard sandbox. Relative paths resolve inside the workspace, but absolute paths can reach other host locations unless sandboxing is enabled. See [Sandboxing](/gateway/sandboxing).
...
> **Note:** Direct chats collapse to the agent's **main session key**, so true isolation requires **one agent per person**.
...
```json5
    {
      ...
      bindings: [
        ...
        { agentId: "alerts", match: { channel: "tel