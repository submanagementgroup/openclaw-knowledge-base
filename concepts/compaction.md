---
domain: concepts
topic: "Context Compaction: Managing Long Conversations"
type: concept
keywords:
  - compaction
  - context compaction
...
> **Note:** Before compacting, OpenClaw automatically reminds the agent to save important notes to [memory](/concepts/memory) files. This prevents context loss.
...
> **Note:** The byte guard requires `truncateAfterCompaction: true`. Without transcript rotation, the active file would not shrink and the guard remains inactive.
...
> **Note:** If the provider fails or returns an empty result, OpenClaw falls back to built-in LLM summarization.
