---
domain: concepts
topic: "Agent Loop: Execution Flow, Streaming, Tool Events, and Session Locking"
type: concept
keywords:
  - agent loop
  - execution flow
...
related:
  ...
  - concepts/session
  - concepts/streaming
source: concepts/agent-loop.md
---

The OpenClaw agent loop is the complete execution path for a single agent turn: intake → context assembly → model inference → tool execution → streaming replies → persistence. Each loop is serialized per session.
...
- Assistant deltas are buffered into chat `d
